...
(function() {
    ...
    var caseSum = document.getElementById('caseSum')
    caseSum.innerText = `(${sessionStorage.getItem('caseSum')})`
    btn.addEventListener("click", function() {
        ajax({
            method: 'GET',
            url: baseUrl + '/api/sentences',
    ...
    for (var i = 0; i < btnDelea.length; i++) {
        btnDelea[i].onclick = function() {
            ...
            tr.parentNode.removeChild(tr)
                //重新计算商品总价
            getTotal()
            sessionStorage.setItem('caseSum', sessionStorage.getItem('caseSum') - 1)
            caseSum.innerText = `(${sessionStorage.getItem('caseSum')})`
        }
    }
    // 删除被选中的上哦怕
